...
public class MainActivity extends AppCompatActivity {
    ...
    private double[] flushDuretion = {1, 1+10/60, 1+25/60};
    ...
    private int volumeTime(int coreFrequency, int volumeData, double flushDuretion) {
        return (int)Math.round(volumeData * flushDuretion / coreFrequency);
    }
    ...
    private int coreTime(int[] delayMinute, int[] volumeData, int frequency) {
        ...
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i], flushDuretion[i]); // определение времени обработки компьютером данных от одного спутника
        }
